...
static HAL_StatusTypeDef GT911_Read(uint16_t reg, uint8_t *buf, uint16_t len)
{
    ...
    st = HAL_I2C_Master_Transmit(gt911_i2c, gt911_addr, regbuf, 2, 50);
    if (st != HAL_OK) return st;

    return HAL_I2C_Master_Receive(gt911_i2c, gt911_addr, buf, len, 50);
}
...
static HAL_StatusTypeDef GT911_Write(uint16_t reg, const uint8_t *buf, uint16_t len)
{
    ...
    return HAL_I2C_Master_Transmit(gt911_i2c, gt911_addr, tmp, len + 2, 50);
}
