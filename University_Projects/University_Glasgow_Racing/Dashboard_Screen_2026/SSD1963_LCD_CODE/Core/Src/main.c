...
/* USER CODE BEGIN PV */
    ...
    static Dashboard dashboard_can_sim_tx;
#endif
...
#define CAN_ID_DASHBOARD      0x100 //ADDRESS!!!!!!!!!!!!!!!!!!!!!!!!===============================
#define CAN_DASHBOARD_DLC     8 //Length of data
...
#define DASH_IDX_LAP          5
#define DASH_IDX_THROTTLE     6
#define DASH_IDX_BRAKE        7
...
// screen reading temps
static volatile uint8_t updated = 0;
static enum Screen_State screen_state = STARTING_SCREEN;
static enum TouchMode touch_mode = TOUCH_MODE;
...
//CAN PROTOTYPES
uint8_t CAN2_RxQueue_Pop(CAN2_RxMessage *msg);
uint8_t CAN2_RxQueue_Push(const CAN_RxHeaderTypeDef *hdr, const uint8_t *data);
void CAN2_Start(void);
void CAN2_SendSdcState(uint16_t sdc_minus, uint16_t sdc_plus);
void Dashboard_UpdateFromCAN(const CAN2_RxMessage *msg);

#if CAN_TEST_MODE
static uint8_t CAN2_TestSendDashboardFrame(const Dashboard *src);
static void DashboardSim_SendViaLoopbackCAN(uint32_t now_ms);
#endif
...
int main(void)
{
  ...
  /*
   * Absolute first thing after MCU reset:
   * hold LCD reset low and keep LCD control lines inactive.
   * This is for NRST/warm-reset stability.
   */
  LCD_EarlyPinHold_NoHAL();
 ...
 Screen_State_Change(AUTOMATIC_SECOND_SCREEN);
  ...
  /* USER CODE BEGIN WHILE */
    ...
    while (1)
    {
        ...
        if (touch_ok && touch_mode != DISABLED && HAL_GetTick() - last_touch >= 25)
        {
            if (GT911_ReadState(&touch) == HAL_OK)
            {
                if (touch.touched && !last_touch)
                {
                    ...
                    {
                        /*
                        =============WHERE TOUCH IS REGISTERED LOGIC==============
                        ...
                        */
                       /*
                       CURRENTLY FLIPS THROUGH ALL SCREENS
                       */
                        if(screen_state == LOGO) {
                            screen_state = ENDURANCE;
                        }
                        else if(screen_state == ENDURANCE) {
                            screen_state = PEDAL;
                        }
                        else if(screen_state == PEDAL) {
                            screen_state = LOGO;
                        }
                        
                       Screen_State_Change(screen_state);
        ...
        
        {
            ...
            if (HAL_GetTick() - last_adc_sample_ms >= 50)
            {
                ...
                //Call the handler that reads the ADC values
                //Also sends the SDC state over CAN if SEND_SDC_STATE is set to 1
                ADC1_Private_Handler();
            }
        ...
        if (HAL_GetTick() - last_screen_test >= 50)
        {
            ...
            if (CAN_TEST_MODE)
                {
                    /*
                    * Uses real CAN TX + bxCAN internal loopback.
                    * Real d is updated only after HAL RX callback + RX queue + parser.
                    */
                    #if CAN_TEST_MODE
                    DashboardSim_SendViaLoopbackCAN(last_screen_test);
                    #endif
                }
                else
                {
                    /*
                    * Direct dashboard test mode: bypasses CAN.
                    */
        ...
        do
        {
            //Pop a message from the CAN RX queue (disables interrupts to avoid racd condition) 
            __disable_irq();
        ...
        //This switch statement concerns updates to the screen state
        switch(screen_state)
        {
            case ENDURANCE:
                if (updated)
            ...
            case LOGO:
            // No updates needed for logo screen -> static logo
                break;
        }
...
void Dashboard_UpdateFromCAN(const CAN2_RxMessage *msg)
{
    ...
    d.speed             = (uint16_t)msg->data[DASH_IDX_SPEED_L] | ((uint16_t)msg->data[DASH_IDX_SPEED_H] << 8);
    d.lap               = msg->data[DASH_IDX_LAP];
    d.throttle_percent  = msg->data[DASH_IDX_THROTTLE];
    d.brake_percent     = msg->data[DASH_IDX_BRAKE];
}
...
StateRdLevel GetStateRdLevel(uint16_t adc_value)
{
...
void ApplyScreenFromStateRd(StateRdLevel level)
{
...
static uint8_t CAN2_TestSendDashboardFrame(const Dashboard *src)
{
    ...
    /*
     * Build a real CAN TX frame using the dashboard message format.
     * In CAN_MODE_LOOPBACK, HAL_CAN_AddTxMessage() will transmit internally
     * and the frame will appear in RX FIFO like a received CAN frame.
     */
    CAN_TxHeaderTypeDef txHeader;
    uint32_t txMailbox;
    uint8_t txData[8] = {0};
    ...
    txHeader.StdId = CAN_ID_DASHBOARD;
    txHeader.ExtId = 0;
    txHeader.IDE   = CAN_ID_STD;
    txHeader.RTR   = CAN_RTR_DATA;
    txHeader.DLC   = CAN_DASHBOARD_DLC;
    txHeader.TransmitGlobalTime = DISABLE;

    txData[DASH_IDX_BATTERY]    = src->battery_charge;
    txData[DASH_IDX_CELL_TEMP]  = src->cell_temperature;
    txData[DASH_IDX_WATER_TEMP] = src->water_temperature;
    txData[DASH_IDX_SPEED_L]    = (uint8_t)(src->speed & 0xFF);
    txData[DASH_IDX_SPEED_H]    = (uint8_t)((src->speed >> 8) & 0xFF);
    txData[DASH_IDX_LAP]        = src->lap;
    txData[DASH_IDX_THROTTLE]   = src->throttle_percent;
    txData[DASH_IDX_BRAKE]      = src->brake_percent;

    if (HAL_CAN_GetTxMailboxesFreeLevel(&hcan2) == 0) {
        return 0;
    }

    return (HAL_CAN_AddTxMessage(&hcan2, &txHeader, txData, &txMailbox) == HAL_OK) ? 1 : 0;
}

static void DashboardSim_SendViaLoopbackCAN(uint32_t now_ms)
{
    /*
     * Generate random/simulated sender-side data using dashboard_tests.c.
     * This does NOT modify real dashboard d.
     */
    dashboard_test_sim_step(&dashboard_test_sim, &dashboard_can_sim_tx, now_ms);

    /*
     * Send the simulated data through the real CAN peripheral.
     * In CAN_MODE_LOOPBACK, the normal HAL RX callback will receive it.
     */
    (void)CAN2_TestSendDashboardFrame(&dashboard_can_sim_tx);
}
...
void Screen_State_Change(enum Screen_State new_state) 
    {   
...
void ADC1_Private_Handler(void) {
